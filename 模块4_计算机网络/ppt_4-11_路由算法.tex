...
\begin{frame}[fragile]{BGP边界网关协议}
    ...
    \begin{center}
        \begin{tikzpicture}[>=stealth]
            \node[draw,minimum width=2cm,minimum height=1cm,fill=blue!10] (AS1) {AS 100};
            \node[draw,minimum width=2cm,minimum height=1cm,fill=red!10] (AS2) at (3.5,0) {AS 200};
            \node[draw,minimum width=2cm,minimum height=1cm,fill=green!10] (AS3) at (7,0) {AS 300};
            \draw[->] (AS1) -- node[above]{BGP} (AS2);
            \draw[->] (AS2) -- node[above]{BGP} (AS3);
            \node[below] at (2.5,-1.2) {路径: AS100 $\to$ AS200 $\to$ AS300};
        \end{tikzpicture}
    \end{center}
\end{frame}
