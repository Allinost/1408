...
\begin{frame}{RIP --- 慢收敛与计数到无穷（2/2）}
    ...
    \begin{block}{其他优化措施}
        \begin{itemize}
            \item \textbf{毒性逆转}：将不可达路由的跳数设为16直接通告给邻居
            \item \textbf{触发更新}：拓扑变化时立即发送更新，不等30秒周期
            \item \textbf{抑制计时器}：收到某网络不可达信息后，进入抑制状态
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{OSPF --- 区域划分}
    \begin{center}
        \begin{tikzpicture}[>=stealth]
            \node[draw,circle,minimum size=1.2cm,fill=red!20,align=center] (area0) at (0,0) {Area 0\\骨干区域};
