...
\begin{frame}[fragile]{距离向量算法 --- RIP协议}
    \begin{block}{核心思想}
...
\begin{frame}[fragile]{距离向量算法 --- 工作过程}
    \begin{center}
...
\begin{frame}[fragile]{RIP --- 慢收敛与计数到无穷}
    \begin{block}{问题}
...
\begin{frame}[fragile]{链路状态算法 --- OSPF协议}
    \begin{block}{核心思想}
...
\begin{frame}[fragile]{Dijkstra最短路径算法}
    \begin{block}{算法步骤}
        \begin{enumerate}
...
\begin{frame}[fragile]{距离向量 vs 链路状态}
    \begin{table}
        \centering
        \begin{tabular}{|c|c|c|}
            \hline
...
\begin{frame}[fragile]{BGP边界网关协议}
    \begin{block}{BGP的作用}
        在不同自治系统（AS）之间交换路由信息，是互联网的"粘合剂"。
    \end{block}
    \vspace{0.3em}
    \begin{columns}[t]
        \column{0.5\textwidth}
        \textbf{基本概念}
        \begin{itemize}
            \item AS：同一管理机构下的网络集合
            \item 每个AS有唯一AS号（16位/32位）
            \item BGP基于\textbf{路径向量}算法
            \item TCP端口179
        \end{itemize}
        \column{0.5\textwidth}
        \textbf{路径向量}
    ...
    \begin{center}
        \begin{tikzpicture}[>=stealth]
            \node[draw,ellipse,minimum width=2cm,minimum height=1cm,fill=blue!10] (AS1) {AS 100};
            \node[draw,ellipse,minimum width=2cm,minimum height=1cm,fill=red!10,right=of AS1] (AS2) {AS 200};
            \node[draw,ellipse,minimum width=2cm,minimum height=1cm,fill=green!10,right=of AS2] (AS3) {AS 300};
            \draw[<->] (AS1) -- node[above]{BGP} (AS2);
            \draw[<->] (AS2) -- node[above]{BGP} (AS3);
            \node[below] at (AS2.south) {路径: AS100 $\to$ AS200 $\to$ AS300};
        \end{tikzpicture}
    \end{center}
\end{frame}

\begin{frame}[fragile]{路由协议对比总结}
    \begin{table}
        \centering
        \begin{tabular}{|c|c|c|c|}
            \hline
            \textbf{协议} & \textbf{RIP} & \textbf{OSPF} & \textbf{BGP} \\
            \hline
    ...
    \vspace{0.5em}
    \begin{itemize}
        \item IGP：同一AS内部使用的路由协议（RIP、OSPF）
        \item EGP：不同AS之间使用的路由协议（BGP）
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{本节总结}
    \begin{enumerate}
